...
本仕様を実装した後は、LINE STORE 訪問済み状態によるふりがな強調よりも、本書の時計ドラッグ連動演出を正とする。
...
- 自動回転だけでは演出を発火させず、ユーザーがドラッグしているときだけ発火させる
...
たたみ状態の右下ポップ導線に、12 個の数字を持つ時計盤と長針を表示する。
...
時計のドラッグ操作が、たたみ導線全体の展開クリックとして扱われてはならない。時計盤以外の導線領域を押した場合は、従来通り展開できる。
...
自動回転は視覚的な誘導であり、ページ側の強調・きらり演出の発火条件には含めない。
...
タイプ詳細ページでは、対象タイプの `furiganaEmphasisIndex` と時計の `selectedHour` が一致したときだけ、該当ふりがな文字を光らせる。
...
- 強調色は gold 系を基本とする
- 発火中だけアクセントクラスを付け、発火条件を外れたら即座に戻す

## 8. `/` の 16 タイプ一覧演出

トップページでは、16 タイプ一覧の各カードが自身の `furiganaEmphasisIndex` を持つ。

時計ドラッグ中に `selectedHour` とカードの `furiganaEmphasisIndex` が一致した場合、そのカードに「きらり」と光る演出を出す。
...
- `/` で、ドラッグ中かつ対象時刻一致時だけ該当タイプカードがきらりと光ること
- `/` で、対象時刻が一致する複数カードが同時に光ること
- ドラッグ終了後、ふりがな強調とカード発光が解除されること
