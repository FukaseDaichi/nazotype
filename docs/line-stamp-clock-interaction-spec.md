...
- 導線が最小化または閉じる状態でも、対象ページ上では内部時計を継続し、ページ側のきらり演出へ反映する
...
結論として、最小化中や閉じる操作後でも、ページ内のきらり演出を継続することは可能である。

時計盤の表示と、時計の内部進行は分離して扱う。`LineStampFloatingPromoClient` が対象ページにマウントされている間は、表示モードが `expanded` でなくても内部角度と `selectedHour` を更新し、外部 store へ publish する。

表示モードごとの扱い:

- `expanded`: 時計盤を表示し、自動回転・ドラッグ操作・キーボード操作を受け付ける
- `collapsed`: 時計盤は表示しないが、内部時計は自動回転を継続し、`selectedHour` を更新する
- `hidden`: 導線 UI は表示しないが、現在のページマウント中は内部時計を継続し、`selectedHour` を更新する

境界条件:

- 閉じる操作による `hidden` は永続保存しないため、再読み込みやページ遷移後は通常の初期表示ルールに戻る
- `/diagnosis`、`/secret/`、LINE スタンプ URL 無効時など、導線コンポーネント自体がマウントされない状態では発火しない
- ブラウザタブやアプリ自体が閉じている間は JavaScript が実行されないため、継続対象外とする
- バックグラウンドタブではブラウザにより `requestAnimationFrame` が抑制されるため、再開時に経過時間から角度を補正できる実装にする
- `prefers-reduced-motion: reduce` では、最小化・閉じる状態でも自動回転による継続発火は行わない

実装方針:

- 時計の内部進行を展開カードの DOM 表示に依存させない
- 自動回転の effect は `mode === "expanded"` だけを条件にせず、対象ページにマウントされていることと `isDragging === false` を主条件にする
- `collapsed` / `hidden` への変更時に `selectedHour: null` へリセットしない
- リセットはコンポーネントのアンマウント時、導線無効時、または明示的に時計状態を破棄する必要がある場合に限定する
- ドラッグ操作とキーボード操作は、時計盤が見える `expanded` のときだけ受け付ける
