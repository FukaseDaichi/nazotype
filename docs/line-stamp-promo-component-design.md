...
`collapsed` / `hidden` でも `selectedHour` を外部 store に publish し続けるため、トップページのタイプカード発光とタイプ詳細ページのメイン画像発光は継続できる。
...
タイプ詳細ページでは、この条件を満たす間だけメインのタイプ画像を光らせる。

タイプ詳細ページのふりがな強調だけは、従来通り `isDragging === true` も条件に加える。

トップページでは、この条件を満たす 16 タイプ一覧カードを一時的にきらりと光らせる。一致するカードが複数ある場合は同時に光ってよい。ドラッグ中でなくても、時計の現在時刻が一致すれば発火する。
