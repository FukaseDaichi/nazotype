...
LINE スタンプ右下ポップ導線の時計連動演出では、`furiganaEmphasisIndex` を 1 から 12 の時計位置として扱う。内部時計がこの値と同じ時刻を指した場合、タイプ詳細ページではメインのタイプ画像を光らせ、トップページでは該当タイプカードをきらりと光らせる。導線が最小化または閉じる状態でも、対象ページに導線コンポーネントがマウントされている間は内部時計を継続してよい。タイプ詳細ページの該当ふりがな文字は、ドラッグ中に同じ時刻を指した場合だけ光らせる。
