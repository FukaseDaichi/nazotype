...
9. LINE スタンプ導線の内部時計が `furiganaEmphasisIndex` と同じ時刻を指した場合、表示中ページに応じた一時演出を出す
...
LINE スタンプ右下ポップ導線の内部時計が各タイプの `furiganaEmphasisIndex` と同じ時刻を指した場合、16 タイプ一覧の該当カードを一時的にきらりと光らせる。ドラッグ中でなくても時刻一致で発火する。導線が最小化または閉じる状態でも、対象ページに導線コンポーネントがマウントされている間は内部時計を継続し、この演出を出してよい。ただし、カードリンク枠の色変化や枠発光は、時計を pointer drag 中に時刻一致した場合だけ出す。LINE STORE 遷移だけではこの演出を出さない。
...
LINE スタンプ右下ポップ導線の内部時計が表示中タイプの `furiganaEmphasisIndex` と同じ時刻を指した場合、メインのタイプ画像面を一時的に光らせる。導線が最小化または閉じる状態でも、対象ページに導線コンポーネントがマウントされている間は内部時計を継続し、この画像面の発光を出してよい。ただし、メイン画像枠の色変化や枠発光は、時計を pointer drag 中に時刻一致した場合だけ出す。さらに、時計をドラッグ中に同じ条件を満たした場合だけ、その位置のふりがなも一時的に光らせる。LINE STORE 遷移済み状態だけでは、画像発光やふりがな強調を発火させない。
...
- 共有キーから復元した `typeCode` が URL の `typeCode` と一致しない場合は結果表示を追加しない
...
- `NEXT_PUBLIC_GA_MEASUREMENT_ID` は GA4 の Measurement ID に使う
- docs 上では `NEXT_PUBLIC_GA_MEASUREMENT_ID` を `G-REDACTED` と表記する
- `NEXT_PUBLIC_GA_MEASUREMENT_ID` 未設定時は GA4 script を読み込まず、診断完了イベントも送信しない
- GA4 では診断完了時に `diagnosis_complete` event と `type_code` parameter だけを送る
...
- 診断結果のタイプ別件数と割合の集計は GA4 の `diagnosis_complete` event で行い、回答個票は保存しない
