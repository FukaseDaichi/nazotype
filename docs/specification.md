...
LINE スタンプ右下ポップ導線の内部時計が各タイプの `furiganaEmphasisIndex` と同じ時刻を指した場合、16 タイプ一覧の該当カードを一時的にきらりと光らせる。ドラッグ中でなくても時刻一致で発火する。導線が最小化または閉じる状態でも、対象ページに導線コンポーネントがマウントされている間は内部時計を継続し、この演出を出してよい。LINE STORE 遷移だけではこの演出を出さない。
...
LINE スタンプ右下ポップ導線の内部時計が表示中タイプの `furiganaEmphasisIndex` と同じ時刻を指した場合、メインのタイプ画像を一時的に光らせる。導線が最小化または閉じる状態でも、対象ページに導線コンポーネントがマウントされている間は内部時計を継続し、この画像発光を出してよい。さらに、時計をドラッグ中に同じ条件を満たした場合だけ、その位置のふりがなも一時的に光らせる。LINE STORE 遷移済み状態だけでは、画像発光やふりがな強調を発火させない。
