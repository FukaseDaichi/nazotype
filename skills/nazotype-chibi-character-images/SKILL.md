...
This skill is **chibi-first**. The primary workflow uses the built-in `image_gen` tool, not the repo's fal.ai scripts.
...
- The legacy fal.ai scripts under `scripts/` remain only for downstream or older workflows. Do not choose them for new chibi base asset work unless the user explicitly asks for that legacy path.
